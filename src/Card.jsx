...
const App = () => {
  ...
  const handleAdd = (e) => {
    ...
    if (isEdit && editId) {
      ...
      setTodos((prev) => [
        ...prev,
        { id: Date.now().toString(), text, status: "todo" },
      ]);
      clear();
    }
  };
  ...
  const moveToProgress = (id) => {
    ...
    if (item) {
      setProgress([...progress, { ...item, status: "inprogress" }]);
      deleteTodo(id);
    }
  };
  ...
  const moveToCompleted = (id, from) => {
    ...
    if (item) {
      setCompleted([...completed, { ...item, status: "completed" }]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-800 p-5 rounded-xl shadow-lg">
          ...
          {todos.map((todo) => (
            <div
              key={todo.id}
              className="flex justify-between items-center bg-gray-700 p-3 rounded-lg mb-3 shadow-md hover:bg-gray-600 transition"
            >
